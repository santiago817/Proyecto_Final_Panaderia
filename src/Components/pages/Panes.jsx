...
export default function Panes() {
  ...
  useEffect(() => {
    ...
    if (carritoGuardado.length > 0) setCarrito(carritoGuardado);
  }, []);
  ...
  useEffect(() => {
    const fetchProductos = async () => {
      try {
        const categoriaRef = doc(db, "categorias", "panes");
  ...
  const reservarProducto = (producto) => {
    setCarrito((prev) => {
      const index = prev.findIndex((p) => p.id === producto.id);
      if (index !== -1) {
        const updated = [...prev];
  ...
  return (
    ...
    >
      {mensajeCarrito && (
        <div className="fixed top-4 right-4 bg-green-400 text-white px-4 py-2 rounded-lg shadow-lg z-50 animate-fade-in">
          {mensajeCarrito}
        </div>
      )}
            ...
            <div className="p-3 flex flex-col gap-2">
              ...
              <div className="flex justify-between items-center mt-2">
                ...
                <button
                  ...
                  onClick={() => reservarProducto(producto)}
                >
                  Reservar
                </button>
              </div>
