...
export default function Facturas() {
  ...
  useEffect(() => {
    const fetchFacturas = async () => {
      try {
        const categoriaRef = doc(db, "categorias", "facturas");
        ...
        if (facturasData.length === 0) setError("No hay facturas disponibles.");
        else {
          setFacturas(facturasData);
          setError("");
          const inicial = {};
          facturasData.forEach((p) => (inicial[p.id] = 0));
          setCantidades(inicial);
        }
      } catch {
        setError("Error al obtener facturas. Revisa Firestore.");
      }
    };
    fetchFacturas();
  }, []);
  ...
  const handleAgregarFactura = async () => {
    ...
    try {
      const categoriaRef = doc(db, "categorias", "facturas");
      const facturasRef = collection(categoriaRef, "productos");
      ...
      const inicial = {};
      facturasData.forEach((p) => {
        inicial[p.id] = cantidades[p.id] || 0;
      });
      setCantidades(inicial);
    } catch {
      alert("Error al agregar factura. Solo admins pueden hacerlo.");
    }
  };
  ...
  const handleBorrarFactura = async () => {
    if (!facturaAEliminar)
      return setDeleteMessage({
        text: "Seleccioná una factura primero",
        type: "error",
      });
    ...
    try {
      ...
      setFacturas((prev) => prev.filter((p) => p.id !== facturaAEliminar));
      setFacturaAEliminar("");
      ...
      {deleteModalOpen && admin && (
        ...
        >
          ...
          >
            ...
            <select
              value={facturaAEliminar}
              onChange={(e) => {
                setFacturaAEliminar(e.target.value);
                setDeleteMessage({ text: "", type: "" });
              }}
        ...
        {facturas.map((factura) => (
          ...
          >
            <div className="relative h-40 w-full overflow-hidden rounded-t-2xl">
              <img
                src={factura.imagen || "https://via.placeholder.com/150"}
                alt={factura.nombre || factura.id}
